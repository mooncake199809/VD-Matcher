...
class LoFTRLoss(nn.Module):
    ...
    def compute_c_weight(self, data):
        ...
        if 'mask0' in data:
            c_weight = (data['mask0'][..., None] * data['mask1'][:, None]).float()
        else:
            c_weight = None
        return c_weight
    ...
    def forward(self, data):
        ...
        b,_,seed_num = data["conf_matrix"].shape
        seed0_index = data['i_seed_idx1_max']	# [2 2048]	i
        seed1_index = data['j_seed_idx1_max']	# [2 2048]	j
        seed_index_i = seed0_index[:,:,None].repeat(1,1,seed_num)	# [2 2048 2048]
        seed_index_j = seed1_index[:,None,:].repeat(1,seed_num,1)	# [2 2048 2048]
        seed_index_b = torch.arange(b)[:,None,None].repeat(1,seed_num,seed_num)
        data['conf_matrix_seed_gt'] = conf_matrix_gt[seed_index_b, seed_index_i, seed_index_j]

        # 1. coarse-level loss
        loss_c = self.compute_coarse_loss(
            data['conf_matrix_with_bin'] if self.sparse_spvs and self.match_type == 'sinkhorn' \
                else data['conf_matrix'],
            data['conf_matrix_seed_gt'],
            weight=c_weight)
